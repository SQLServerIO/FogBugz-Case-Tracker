...
namespace FogBugzNet
{
    ...
    {
        private readonly Dictionary<int, XmlNode> _caseToNode = new Dictionary<int, XmlNode>();
        private readonly XmlDocument _doc = new XmlDocument();
        private readonly FogBugz _fb;
        private readonly Dictionary<int, ProjectNode> _projectIdToNode = new Dictionary<int, ProjectNode>();
        ...
        public XmlElement CaseToNode(Case c)
        {
            ...
            node.Attributes.Append(_doc.CreateAttribute("TEXT")).Value = String.Format("{0} {1}: {2}", c.Category, c.ID,
                                                                                       c.Name);
            node.Attributes.Append(_doc.CreateAttribute("LINK")).Value = _fb.BaseURL + "?" + c.ID;
        ...
        private void GenerateDocumentSkeleton()
        {
            ...
            _rootNode.Attributes.Append(_doc.CreateAttribute("LINK")).Value = _fb.BaseURL +
                                                                              "?pre=preMultiSearch&pg=pgList&pgBack=pgSearch&search=2&searchFor=" +
                                                                              _search.Query;
        ...
        private XmlElement CreateMileStoneNode(Case c)
        {
            ...
            mileStoneNode.Attributes.Append(_doc.CreateAttribute("TEXT")).Value = string.Format("MileStone: {0}",
                                                                                                c.ParentMileStone.Name);
            mileStoneNode.Attributes.Append(_doc.CreateAttribute("LINK")).Value =
                string.Format("{0}?pg=pgList&pre=preSaveFilterFixFor&ixFixFor={1}&ixStatus=-2", _fb.BaseURL,
                              c.ParentMileStone.ID);
        ...
        {
            var ret = new ProjectNode();
            ret.Node = NewElement();
            ret.Node.Attributes.Append(_doc.CreateAttribute("TEXT")).Value = string.Format("Project: {0}",
                                                                                           c.ParentProject.Name);
            ret.Node.Attributes.Append(_doc.CreateAttribute("LINK")).Value =
                string.Format(
                    "{0}?pre=preMultiSearch&pg=pgList&pgBack=pgSearch&search=2&searchFor=status:%22active%22+project:%22{1}%22+OrderBy:%22FixFor%22+View:%22Outline%22",
                    _fb.BaseURL, c.ParentProject.Name);
